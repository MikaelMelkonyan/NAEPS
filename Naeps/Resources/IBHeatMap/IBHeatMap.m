...
- (void)redrawView
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [self calculatePixelColors:(bounds)];
...
- (void)calculatePixelColors:(CGRect)bounds
{
    CGFloat maxDensity = [self maxDensityCalc:(bounds)];
    IBMatrix *matrix = [[IBMatrix alloc]initWithColumns:CGRectGetWidth(bounds) lines:CGRectGetHeight(bounds)];
    for (NSInteger x = 0; x < CGRectGetWidth(bounds); x ++) {
        for (NSInteger y = 0; y < CGRectGetHeight(bounds); y ++) {
...
- (CGFloat)densityForPoint:(CGPoint)point bounds:(CGRect)bounds
{
    CGFloat density = 0;
...
- (CGFloat)maxDensityCalc:(CGRect)bounds
{
    IBMatrix *densityMatrix = [[IBMatrix alloc]initWithColumns:CGRectGetWidth(bounds) lines:CGRectGetHeight(bounds)];
    ...
    dispatch_async(queue, ^{
        for(NSInteger x = 0; x < floor(CGRectGetWidth(bounds) / 2.0f); x++) {
            for(NSInteger y = 0; y < CGRectGetHeight(bounds); y++) {
                CGPoint point = CGPointMake(x, y);
                CGFloat density = [self densityForPoint:point bounds:bounds];
    ...
    dispatch_async(queue, ^{
        for(NSInteger x = floor(CGRectGetWidth(bounds) / 2.0f); x < CGRectGetWidth(bounds); x++) {
            for(NSInteger y = 0; y < CGRectGetHeight(bounds); y++) {
                CGPoint point = CGPointMake(x, y);
                CGFloat density = [self densityForPoint:point bounds:bounds];
                [densityMatrix setObject:@(density) column:x line:y];
